...
export const PriceCard = ({ price, saved, percentage, title, description, cta, size, clicked }: PriceCardProps): ReactElement => {
  return (
    <div className={`w-full ${size === 'sm' ? 'h-[600px] lg:h-full bg-[#40403e]' : size === 'en' ? 'h-[600px] lg:h-full' : 'h-[600px] lg:h-4/5 bg-[#282826] '} flex flex-col justify-between items-start p-5 rounded-[15px]`}>
      ...
      <div className="w-full flex flex-col gap-y-3">
        ...
        <div className="w-full flex max-[1370px]:flex-col justify-between items-end max-[1370px]:gap-y-2 gap-x-1 pt-1">
        ...
        <div className="w-full flex justify-between items-end gap-x-1 pt-5">
          <p className={`text-[1.1rem]  lg:text-[14px] text-balance ${size === 'en' ? 'text-gray-600 font-body' : 'text-gray-300 font-light'} leading-none lg:w-2/3`}>
            {description}
          </p>
        </div>
      </div>
