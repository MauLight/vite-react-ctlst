  ...
  return (
    <div className={`fixed bottom-10 left-0 ${expandNav ? 'w-full' : 'w-[223px]'} h-[60px] flex justify-center transition-all duration-200 z-20`}>
      <div className="relative rounded-[8px] overflow-hidden flex mx-20">
        <div className={`flex gap-x-2 w-full h-full z-10 p-2 ${step === 2 ? 'animated-background bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500' : ''}`}>
          ...
          {
            step === 2 ? (
            <div className="flex gap-x-2 bg-[#40403e] rounded-[5px]">
            <div
                className={`w-[600px] flex justify-center items-center animated-background bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 p-[2px] rounded-[5px] transition-all duration-200`}>
                <input
                  value={creatorInputValue} onChange={({ target }) => { setCreatorInputValue(target.value) }} onKeyDown={handleEnterInput} placeholder={'Type /screenplay to start writing. Type /help to see all the commands.'} type="text" className={`${isCreator ? 'animate-pulse' : ''} w-full h-10 px-2 rounded-[3px] bg-[#f3f3f3] ring-0 focus:ring-0 focus:outline-none text-[0.9rem]`} />
              </div>
            </div>
            )
            :
