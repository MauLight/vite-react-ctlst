              ...
              <div className="relative w-[320px] min-h-[300px] bg-[#ffffff] flex flex-col rounded-[5px] px-5 pt-20 pb-4 gap-y-4">
                ...
                <div className='flex flex-col gap-y-3'>
                  <div className='relative w-full h-[50px] border border-indigo-500 rounded-[5px] px-2 overflow-hidden'>
                    <label htmlFor='email' className={`absolute ${labelAnimationEmail || fieldValueEmail.length > 0 ? 'top-1 pl-2 text-[12px] text-indigo-400' : 'top-3 pl-2 text-[16px] text-gray-600'} font-light transition-all duration-200`}>Email</label>
                    <input id='email' value={fieldValueEmail} onChange={({ target }) => { setFieldValueEmail(target.value) }} onMouseDown={() => { setLabelAnimationEmail(true) }} onBlur={() => { setLabelAnimationEmail(false) }} type="email" className='absolute h-8 flex items-end font-light text-[0.9rem] text-gray-600 bg-transparent bottom-0 px-2 w-full ring-0 focus:ring-0 focus:outline-none' />
                  </div>
                  ...
                  {
                    currStep === 1 ? (
                      <button onClick={handleNextStep} className='w-full h-10 bg-indigo-500 text-[#ffffff] rounded-[5px]'>Next</button>
                    )
                      :
                      (
